...
export default function ThaiAddressRow({ address, onChange, onAddressSelect }: Props) {
  ...
  const handleSearch = async (query: string, field: 'sub_district' | 'district' | 'province' | 'postal_code') => {
    ...
    <div className="grid grid-cols-2 gap-6 relative" ref={wrapperRef}>
      ...
      <div className="col-span-2 md:col-span-1">
        ...
        <input 
          ...
          onChange={e => handleSearch(e.target.value, 'postal_code')}
          onFocus={() => { if(suggestions.length > 0) setShowSuggestions(true) }}
          className="w-full px-4 py-3 bg-surface border border-foreground/10 rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-foreground transition-all" 
        />
      </div>
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div className="absolute top-full left-0 right-0 z-50 mt-2 bg-surface border border-foreground/10 shadow-2xl rounded-2xl overflow-hidden backdrop-blur-xl">
          {suggestions.map((s, i) => (
            <button
              key={i}
            ...
            >
              ...
              <span className="font-mono font-bold text-primary/70">{s.postal_code}</span>
            </button>
          ))}
        </div>
      )}
