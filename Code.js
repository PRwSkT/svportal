...
function getMetaConfig_(targetPage) {
  ...
  if (targetPage === 'football') {
    config.pageId = "1204516039414311";
  } else if (targetPage === 'swimming') {
    config.pageId = "1188473751019404";
  } else if (targetPage === 'tutoring') {
    config.pageId = "1235898419607130";
  }
  
  // แปลง System User Token เป็น Page Access Token ที่ตรงกับ Page ID แบบอัตโนมัติ
  try {
    var tokenUrl = "https://graph.facebook.com/v20.0/" + config.pageId + "?fields=access_token&access_token=" + sysToken;
    var tokenRes = UrlFetchApp.fetch(tokenUrl, { "muteHttpExceptions": true });
    var tokenData = JSON.parse(tokenRes.getContentText());
    if (tokenData.access_token) {
      config.token = tokenData.access_token;
    }
  } catch (e) {
    // Fallback to original token if fetch fails
  }
